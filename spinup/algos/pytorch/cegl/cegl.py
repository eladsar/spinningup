...
def sample_ellipsoid(S, z_hat, m_FA, Gamma_Threshold=1.0, min_dist=1e-2):
    ...
    R = min_dist + (1 - min_dist) * torch.ones(bz, nz, 1, device=z_hat.device) * (
        torch.rand(bz, 1, m_FA, device=z_hat.device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', type=str, default='egl', help='[egl|sac]')
    ...
    cegl(lambda: gym.make(args.env), actor_critic=core.MLPActorCritic,
         ac_kwargs=dict(hidden_sizes=[args.hid] * args.l),
         gamma=args.gamma, seed=args.seed, epochs=args.epochs,
         logger_kwargs=logger_kwargs, eps=args.eps, n_explore=args.n_explore,
         device=args.device, batch_size=args.batch_size, architecture=args.architecture, sample=args.sample, method=args.method)
